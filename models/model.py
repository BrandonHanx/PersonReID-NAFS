...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.fc_p2_list = nn.ModuleList(
            [nn.Linear(inp_size, args.feature_size) for i in range(self.part2)]
        )
        self.fc_p3_list = nn.ModuleList(
            [nn.Linear(inp_size, args.feature_size) for i in range(self.part3)]
        )

        self.fc_p2_list_query = nn.ModuleList(
            [nn.Linear(args.feature_size, args.feature_size) for i in range(self.part2)]
        )
        self.fc_p2_list_value = nn.ModuleList(
            [nn.Linear(args.feature_size, args.feature_size) for i in range(self.part2)]
        )

        self.fc_p3_list_query = nn.ModuleList(
            [nn.Linear(args.feature_size, args.feature_size) for i in range(self.part3)]
        )
        self.fc_p3_list_value = nn.ModuleList(
            [nn.Linear(args.feature_size, args.feature_size) for i in range(self.part3)]
        )
    ...
    def forward(
        ...
        b1, part_feature_list_b2, part_feature_list_b3 = self.image_model(
            images, p2, p3
        )
        text_features = self.language_model(tokens, segments, input_masks)

        global_img_feat, global_text_feat = self.build_joint_embeddings(
            b1, text_features[:, 0]
        )
        global_img_feat = self.bottleneck_image(global_img_feat)
        global_text_feat = self.bottleneck_text(global_text_feat)

        local_text_feat = torch.cat(
            (global_text_feat.unsqueeze(1), local_text_feat, text_features[:, 1:99]),
            dim=1,
        )
        ...
        local_img_query = torch.zeros(
            global_img_feat.shape[0], self.part2 + self.part3, global_img_feat.shape[1]
        ).cuda()
        local_img_value = torch.zeros(
            global_img_feat.shape[0], self.part2 + self.part3, global_img_feat.shape[1]
        ).cuda()
        ...
        for i in range(len(part_feature_list_b3)):
            local_img_query[:, i + self.part2, :] = self.fc_p3_list_query[i](
                part_feature_list_b3[i]
            )

        for i in range(len(part_feature_list_b2)):
            local_img_value[:, i, :] = self.fc_p2_list_value[i](part_feature_list_b2[i])
        for i in range(len(part_feature_list_b3)):
            local_img_value[:, i + self.part2, :] = self.fc_p3_list_value[i](
                part_feature_list_b3[i]
            )

        local_img_query = torch.cat(
            (global_img_query.unsqueeze(1), local_img_query), dim=1
        )
        local_img_value = torch.cat(
            (global_img_value.unsqueeze(1), local_img_value), dim=1
        )
