...
def func_attention_MxN(
    local_img_query, txt_i_key_expand, txt_i_value_expand, opt, eps=1e-8
):
    """
    ...
    batch_size, queryL, sourceL = (
        txt_i_key_expand.size(0),
        local_img_query.size(1),
        txt_i_key_expand.size(1),
    )
...
class Loss(nn.Module):
    ...
    def compute_weiTexts(
        ...
        for i in range(n_txt):
            ...
            weiText = func_attention_MxN(
                local_img_query, txt_i_key_expand, txt_i_value_expand, args
            )
            ...
            weiImage = func_attention_MxN(
                txt_i_key_expand, local_img_query, local_img_value, args
            )
    ...
    def contrastive_loss(self, i2t_similarites, t2i_similarities, labels):
        ...
        i2t_loss = i2t_pred * (
            F.log_softmax(i2t_similarites * self.args.lambda_softmax, dim=1)
    ...
    def forward(
        ...
        if self.CMPM:
            cmpm_loss, pos_avg_sim, neg_avg_sim = self.compute_cmpm_loss(
                global_img_feat, global_text_feat, labels
            )
        if self.CMPC:
            cmpc_loss, image_precision, text_precision = self.compute_cmpc_loss(
                global_img_feat, global_text_feat, labels
            )
        if self.CONT:
            i2t_sim, t2i_sim = self.compute_weiTexts(
                local_img_query,
                local_img_value,
                local_text_key,
                local_text_value,
            ...
            cont_loss, local_pos_avg_sim, local_neg_avg_sim = self.contrastive_loss(
                i2t_sim, t2i_sim, labels
            )
...
def compute_topk(
    ...
    for i in range(0, query.shape[0], 200):
        i2t_sim, t2i_sim = Loss.compute_weiTexts(
            query[i : i + 200],
            value_bank[i : i + 200],
            gallery_key,
            gallery_value,
    ...
    global_result.extend(
        topk(sim_cosine_global, target_gallery, target_query, k=k_list)
    )
    if reverse:
        global_result.extend(
            topk(
    ...
    result.extend(
        topk(sim_cosine_all, target_gallery, target_query, k=k_list, reid_sim=reid_sim)
    )
    if reverse:
        result.extend(
            topk(
...
def jaccard(a_list, b_list):
    return (
        1.0
        - float(len(set(a_list) & set(b_list)))
        / float(len(set(a_list) | set(b_list)))
        * 1.0
    )
