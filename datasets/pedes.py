...
class CuhkPedes(data.Dataset):
    ...
    def get_data_from_json(self):
        args = Namespace(
            min_word_count=self.min_word_count, remove_stopwords=None, out_root=None
        )
        ...
        split_metadata = preprocess.process_metadata(
            self.split, split_data, args, write=False
        )
        split_decodedata = preprocess.process_decodedata(split_metadata, vocab)
        data = preprocess.process_dataset(
            self.split, split_decodedata, args, write=False
        )
    ...
    def read_data(self, data):
        ...
        if self.split == "train":
            ...
            raise RuntimeError(
                'Wrong split which should be one of "train","val" or "test"'
            )
