...
from config import data_config, get_image_unique, network_config
...
def train(
    epoch, train_loader, network, optimizer, compute_loss, args, co_location_loss=None
):
    ...
    for step, (images, captions, labels) in enumerate(train_loader):
        ...
        (
            ...
            sep_input_masks,
            sep_caption_length,
        ) = network.module.language_model.pre_process(sep_captions)
        ...
        if step % 10 == 0:
            print(
                "epoch:{}, step:{}, cmpm_loss:{:.3f}, cmpc_loss:{:.3f}, cont_loss:{:.3f}, pos_sim_avg:{:.3f}, neg_sim_avg:{:.3f}, lpos_sim_avg:{:.3f}, lneg_sim_avg:{:.3f}".format(
                    epoch,
                    step,
                    cmpm_loss,
                    cmpc_loss,
